...
const artistController=async(req,res)=>{
    try {
        ...
        const artist_songs = await Songs.find({ artist_name: { $regex: artistKeyword, $options: 'i' } });
...
const songController=async(req,res)=>{
    try {
        ...
        const songs = await Songs.find({ song_name: { $regex: songKeyword, $options: 'i' } });
        if(songs.length==0){
...
const albumController=async(req,res)=>{
    try {
        ...
        if(albumKeyword!=undefined){
            const songs = await Songs.find({ album_name: { $regex: albumKeyword, $options: 'i' } });
